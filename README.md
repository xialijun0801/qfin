...
Linear regression is the most heavily tested topic in statistics. Unfortunately, there is no single book with all need-to-know knowledge. Greene's [Econometric Analysis](http://www.amazon.com/Econometric-Analysis-7th-William-Greene/dp/0131395386) * might be an overshoot due to its length. Honestly, if you are able to manually calculate all outputs from R regression functions (see my comment about R), you should be fine.
